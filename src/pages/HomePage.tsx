 ...
 useEffect(() => {
  ...
  return (
    ...
    >
      {backdrop}
      <FeatureIntro
        heading="Supercharged Instagram Thumbnail Downloader⚡️"
      ...
      />
      <div className="flex flex-col items-center border obrder-gray-500 shadow-lg p-4">
        <TextField
          fullWidth
          value={videoUrl}
          onChange={handleChange}
          id="url-input"
          label="Enter or Paste Instagram Reel/Video Link Here"
          variant="outlined"
        />
        <Button
        ...
        <h3 className="text-xs text-center w-80 m-2 p-2">
          A direct prompt to download thumbnail will get triggered if video has
      ...
      {isDownloadSuccess && (
        <div className="border-2 text-center border-blue-500 shadow-sm p-4">
          ...
          {audioResponse.data.map((format, index) => {
            return (
              <Thumbnail
                thumbnail_link={format.thumbnail_link}
                key={index}
                download_link={format.download_link}
              />
            );
          })}
        </div>
